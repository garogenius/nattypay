...
const UtilityBillUploadModal: React.FC<UtilityBillUploadModalProps> = ({
  isOpen,
  onClose,
  onSubmit,
  initialData,
  ...
  const [fileUrl, setFileUrl] = useState<string>(initialData?.utilityBillUrl || "");
  const [submitting, setSubmitting] = useState(false);
  const fileInputRef = useRef<HTMLInputElement>(null);

  React.useEffect(() => {
    if (isOpen) {
      setFileUrl(initialData?.utilityBillUrl || "");
    ...
    if (file) {
      ...
      if (extension === "pdf" || extension === "jpg" || extension === "jpeg" || extension === "png") {
        ...
        ErrorToast({
          ...
          descriptions: ["Selected file is not a supported format (PDF, JPEG, JPG, or PNG)."],
        });
      }
    }
    ...
    <div className="z-[999999] overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 flex justify-center items-center w-full md:inset-0 h-[100dvh]">
      ...
      <div className="relative mx-2.5 2xs:mx-4 bg-bg-600 dark:bg-bg-1100 border border-border-800 dark:border-border-700 px-0 pt-4 pb-5 w-full max-w-md max-h-[92vh] rounded-2xl overflow-hidden">
        ...
        <div className="px-5 sm:px-6 pt-1 pb-3">
          <h2 className="text-white text-base sm:text-lg font-semibold">Upload Utility Bill</h2>
          <p className="text-white/60 text-sm">Upload your utility bill document for address verification</p>
        </div>

        <div className="px-5 sm:px-6 pb-2 space-y-4 overflow-y-auto max-h-[calc(92vh-200px)]">
          {/* File Upload */}
          <div>
            <label className="block text-sm text-white/80 mb-1.5">Utility Bill Document <span className="text-red-500">*</span></label>
            <div className="w-full flex flex-col gap-3">
              <div className="relative w-full flex items-center bg-bg-2400 dark:bg-bg-2100 border border-border-600 rounded-lg py-3.5 px-3">
                ...
                >
                  ...
                  <span>{selectedFile ? selectedFile.name : "Upload Utility Bill (PDF, JPG, PNG)"}</span>
                </button>
              </div>
              {fileUrl && (
