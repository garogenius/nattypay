...
const UpdateAddressModal: React.FC<UpdateAddressModalProps> = ({ isOpen, onClose, onSubmit }) => {
  const [address, setAddress] = React.useState("");
  const [submitting, setSubmitting] = React.useState(false);

  React.useEffect(()=>{ setAddress(""); }, [isOpen]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      setSubmitting(true);
      if (onSubmit) await onSubmit(address.trim());
      onClose();
    } finally {
      setSubmitting(false);
    }
    ...
    <div className="z-[999999] overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 flex justify-center items-center w-full md:inset-0 h-[100dvh]">
      ...
      <div className="relative mx-2.5 2xs:mx-4 bg-bg-600 dark:bg-bg-1100 border border-border-800 dark:border-border-700 px-0 pt-4 pb-5 w-full max-w-md max-h-[92vh] rounded-2xl overflow-hidden">
        ...
        <div className="px-5 sm:px-6 pt-1 pb-3">
          <h2 className="text-white text-base sm:text-lg font-semibold">Residential Address</h2>
          <p className="text-white/60 text-sm">Edit your residential details</p>
        </div>

        <div className="px-5 sm:px-6 pb-2">
          <label className="block text-sm text-white/80 mb-1.5">New Address</label>
          <div className="w-full flex items-center bg-bg-2400 dark:bg-bg-2100 border border-border-600 rounded-lg py-3.5 px-3">
            <input
              type="text"
              placeholder="Enter new residential address"
