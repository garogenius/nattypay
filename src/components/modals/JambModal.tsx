...
import {
  ...
  usePayForJamb,
} from "@/api/jamb/jamb.queries";
import SpinnerLoader from "@/components/Loader/SpinnerLoader";
import ErrorToast from "@/components/toast/ErrorToast";
import SuccessToast from "@/components/toast/SuccessToast";
import useUserStore from "@/store/user.store";

interface JambModalProps {
  isOpen: boolean;
  onClose: () => void;
}

const JambModal: React.FC<JambModalProps> = ({ isOpen, onClose }) => {
  ...
  // Fetch JAMB plans
  const { jambPlans, isPending: plansLoading } = useGetJambPlans();

  // Fetch bill info when biller is selected
  const { billInfo, isLoading: billInfoLoading } = useGetJambBillInfo({
  ...
  const { mutate: verifyBillerNumber, isPending: verifying } = useVerifyJambBillerNumber(
    onVerifyError,
    onVerifySuccess
  );
  ...
  const { mutate: payJamb, isPending: paying } = usePayForJamb(
    onPayError,
    onPaySuccess
  );
    ...
    <div className="z-[999999] overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 flex justify-center items-center w-full md:inset-0 h-[100dvh]">
      ...
      <div className="relative mx-4 bg-bg-600 dark:bg-bg-1100 border border-border-800 dark:border-border-700 w-full max-w-md rounded-2xl overflow-visible">
        ...
        <div className="flex items-center justify-between p-4 pb-2">
          <div>
            <h2 className="text-white text-lg font-semibold">{step === "form" ? "JAMB" : step === "confirm" ? "JAMB" : "Transaction History"}</h2>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-2 relative" ref={billerRef}>
                ...
                <div onClick={() => setBillerOpen(!billerOpen)} className="w-full bg-bg-2400 dark:bg-bg-2100 border border-border-600 rounded-lg py-3 px-4 text-white text-sm outline-none cursor-pointer flex items-center justify-between">
                  <span className={selectedBiller ? "text-white" : "text-white/50"}>{selectedBiller?.name || "Select JAMB biller"}</span>
                ...
                {billerOpen && (
                  <div className="absolute top-full left-0 right-0 mt-1 z-[100]">
                    <div className="bg-bg-600 dark:bg-bg-2100 border border-border-800 dark:border-border-700 rounded-lg shadow-lg overflow-hidden max-h-60 overflow-y-auto">
                      ...
                      ) : !Array.isArray(jambPlans) || jambPlans.length === 0 ? (
